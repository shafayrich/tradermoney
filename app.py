"""
TraderMoney v14.1 – Expanded Help section. macOS & Windows, ADX + Volume confirmation, port‑based lock.
"""

import json, os, queue, signal, sys, socket, threading, time, traceback, atexit
    ...
    def connect(self):
        ...
        try:
            ...
            if "unauthorized" in msg.lower() or "403" in msg:
                self.ui_queue.put(("error", f"Alpaca authentication failed. URL: {base_url}, Paper: {paper}. Check keys."))
            else:
                self.ui_queue.put(("error", f"Alpaca connection error: {msg}"))
            return False
...
class IBKRBroker(BaseBroker):
    ...
    def stream_prices(self, symbols, callback):
        ...
        def on_tick(ticker):
            if ticker.contract.symbol in symbols and ticker.last is not None:
                callback(ticker.contract.symbol, ticker.last)
...
class TradingViewBroker(BaseBroker):
    def __init__(self, config, ui_queue): super().__init__(config, ui_queue); self.name = "TradingView"; self._stop_server = False; self._webhook_queue = queue.Queue()
    def connect(self):
        ...
        def run():
            ...
            def webhook():
                data = request.json
                if data: self._webhook_queue.put(data)
        ...
        def process():
            while not self._stop_server:
                try:
                    ...
                    sym, price = data.get("symbol"), data.get("price")
                    if sym and price: callback(sym, float(price))
                except queue.Empty: pass
        threading.Thread(target=process, daemon=True).start()
    def stop_stream(self): self._stop_server = True

# ---------- INDICATOR CALCULATOR (enhanced) ----------
class IndicatorCalculator:
    ...
    def compute_all(df, ema_fast=9, ema_slow=50):
        ...
        # Volume confirmation (ratio of current volume to 20‑period average)
...
class SignalAnalyzer:
    ...
    def generate_signal(df, prev_ema_fast, prev_ema_slow, config):
        if prev_ema_fast is None or prev_ema_slow is None:
            return None, ""
        latest = df.iloc[-1]
        ema_f = SignalAnalyzer._safe_float(latest['EMA_fast'])
        ema_s = SignalAnalyzer._safe_float(latest['EMA_slow'])
        ...
        if crossover_bull:
            if not SignalAnalyzer._confirm(df, config, "bull", price):
                return None, ""
            return "BUY", f"BUY @ ${price:.2f}"
        if crossover_bear:
            if not SignalAnalyzer._confirm(df, config, "bear", price):
                return None, ""
    ...
    def _confirm(df, config, direction, price):
        ...
        if config.get('use_adx', True) and adx < ADX_TREND_THRESHOLD:
            return False
        if config.get('use_vol_confirm', True) and vol_ratio < VOLUME_RATIO_THRESHOLD:
            return False
        ...
        if token and chat:
            try:
                ...
                requests.post(f"https://api.telegram.org/bot{token}/sendMessage",
                              json={"chat_id":chat, "text":message, "parse_mode":"HTML"}, timeout=5)
            except: pass
    def run(self):
                ...
                if now - last_hist > 60:
                    ...
                    for sym in self.symbols:
                        ...
                        try:
                            ...
                            self.ui_queue.put(("error", f"Indicator error {sym}: {ie}"))
                            continue
                                    ...
                                    if signal_type == "BUY" and self.positions.get(sym,0)==0:
                                        try:
                                            success = self.broker.submit_order(sym, qty, "buy", "market",
                                                                              sl_pct if use_bracket else None,
                                                                              tp_pct if use_bracket else None)
                                            if success:
...
def start_bot():
    ...
    if broker_choice == "Alpaca":
        state.broker_instance = AlpacaBroker(state.config, state.ui_queue)
    elif broker_choice == "Interactive Brokers":
        state.broker_instance = IBKRBroker(state.config, state.ui_queue)
    else:
        state.broker_instance = TradingViewBroker(state.config, state.ui_queue)
    ...
    while not state.ui_queue.empty():
        try:
            ...
            if msg[0] == "account":
                ...
                state.dashboard["equity"] = eq
                state.dashboard["pl"] = pl
                state.dashboard["buying_power"] = bp
                state.dashboard["open_positions"] = open_pos
            elif msg[0] == "signal":
                ...
                state.dashboard["log"].append(msg[1])
            elif msg[0] == "error":
                state.dashboard["log"].append(f"❌ {msg[1]}")
            elif msg[0] == "ema_update":
                state.dashboard["ema_values"] = msg[1]
        except queue.Empty:
            break
    for key in ["signals", "orders", "log"]:
        if len(state.dashboard[key]) > 50:
            state.dashboard[key] = state.dashboard[key][-50:]
    return jsonify({
        "running": state.running,
        **state.dashboard
    })

# ---------- FRONTEND HTML (expanded Help tab) ----------
FRONTEND_HTML = r"""
<!DOCTYPE html>
<html>
<head>
...
<div id="sidebar">
  <h2>💸 TraderMoney</h2>
  <label>Broker</label>
  <select id="broker-select"><option>Alpaca</option><option>Interactive Brokers</option><option>TradingView</option></select>
...
<div id="main">
  ...
  <div id="tab-ema" class="tab-content">
    <div class="ema-monitor" id="ema-monitor">Loading...</div>
  </div>
  <!-- ========== EXPANDED HELP TAB ========== -->
  <div id="tab-help" class="tab-content">
    <div class="help-content">
      <h2>📘 TraderMoney Help & Documentation</h2>
      <p>Welcome to TraderMoney, your institutional‑grade trading terminal. This guide covers everything you need to configure and run the bot successfully.</p>

      <h3>🚀 Quick Start</h3>
      <ol>
        <li><strong>Choose a Broker</strong> from the dropdown (Alpaca is the simplest for paper trading).</li>
        <li>Enter your <strong>API credentials</strong> (they will be encrypted and saved locally).</li>
      ...
      <p>The bot looks for <strong>EMA crossover</strong> events (fast EMA crosses above/below slow EMA). Every potential signal then passes through multiple confirmation filters to reduce false trades:</p>
      <ul>
        ...
        <li><strong>MACD</strong> – must agree with crossover direction (MACD line vs signal line).</li>
        <li><strong>VWAP</strong> – price must be above VWAP for buys, below for sells.</li>
        <li><strong>Bollinger Bands</strong> – price must be above lower band for buys, below upper band for sells.</li>
        <li><strong>ADX (Trend Strength)</strong> – ADX must be ≥ 20, meaning a clear trend exists (avoids whipsaws in range‑bound markets).</li>
        <li><strong>Volume Confirmation</strong> – current bar's volume must exceed 1.5× the 20‑period average, ensuring strong participation.</li>
      </ul>
      <p><em>All filters can be toggled individually in the sidebar under "Indicators".</em></p>

      <h3>🎛️ Modes Explained</h3>
      <p><strong>Signal Only</strong>: Alerts appear in the Signals tab, are logged, and can be sent via Telegram. No orders are placed.</p>
      <p><strong>Auto Trade</strong>: Valid signals automatically execute market orders through your connected broker. <em>Use with caution – always test in a paper account first.</em></p>

      <h3>🛡️ Risk Management</h3>
      <ul>
        <li><strong>Bracket Orders (SL/TP)</strong> – When enabled, each market order is wrapped with a Stop‑Loss and Take‑Profit limit order. The percentages are configurable (default 2% / 4%).</li>
        <li><strong>Quantity</strong> – Number of shares/contracts per trade.</li>
        <li><strong>Kill Switch</strong> – Immediately closes all open positions with a single click. Works in any mode (broker‑dependent).</li>
      </ul>

      <h3>🏦 Broker Setup</h3>
      <h4>Alpaca</h4>
      <p>Get free API keys at <a href="https://alpaca.markets/" target="_blank" style="color:var(--accent);">alpaca.markets</a>. Toggle "Paper Trading" to use a simulated account. The base URL is chosen automatically.</p>
      <h4>Interactive Brokers</h4>
      <p>Requires TWS or IB Gateway running locally. Fill in host (default 127.0.0.1), port (7497 for paper, 7496 for live), and client ID.</p>
      <h4>TradingView</h4>
      <p>Use the built‑in webhook server (port 5000). Point your TradingView alerts to <code>http://your_ip:5000/webhook</code> with a JSON payload <code>{"symbol":"AAPL","price":185.5}</code>. No credentials needed on this app.</p>

      <h3>📈 Tabs Overview</h3>
      <ul>
        <li><strong>Charts</strong> – Live TradingView chart with symbol switching. Top bar shows account metrics.</li>
        <li><strong>Signals</strong> – Real‑time list of triggered signals with price and rationale.</li>
        <li><strong>History</strong> – Completed trades (only in Auto mode).</li>
        <li><strong>EMA Monitor</strong> – Current EMA values for all tracked tickers.</li>
        <li><strong>Help</strong> – You’re reading it now!</li>
      </ul>
      <p>Tabs can be <strong>dragged</strong> to reorder them (just grab and drop).</p>

      <h3>🔔 Telegram Alerts</h3>
      <p>Enter your bot token and chat ID in the sidebar. You will receive start/stop notifications and every signal in real time. Leave blank to disable.</p>

      <h3>💾 Data & Security</h3>
      <p>All credentials are encrypted with <code>Fernet</code> and stored in <code>~/.tradermoney_config.enc</code>. The encryption key is auto‑generated in <code>~/.tradermoney.key</code>.</p>

      <h3>🌐 Cross‑Platform Notes</h3>
      <p>The app runs natively on macOS and Windows. Only one instance can run at a time (locked by port 5050). Built with Python, Flask, pywebview, and TradingView widgets.</p>

      <h3>🛠️ Troubleshooting</h3>
      <ul>
        <li><strong>No signals?</strong> Check market hours, ensure timeframe/indicators aren’t too restrictive. Try toggling ADX or Volume Confirmation off temporarily.</li>
        <li><strong>Alpaca auth error?</strong> Verify API keys and paper/live switch.</li>
...
Sortable.create(tabHeader, {
  ...
  onEnd: function () {
    ...
    if (!document.querySelector('.tab-btn.active') && first) {
      first.click();
    }
  }
});
...
function switchTab(name, ev) {
  ...
  if (name === 'charts' && chartWidget) {
    setTimeout(() => chartWidget.resize && chartWidget.resize(), 100);
  }
}
...
async function initUI(cfg) {
  ...
  updateCredFields();
  if (cfg.alpaca) {
    const ak = document.getElementById('alpaca-key'); if (ak) ak.value = cfg.alpaca.api_key || '';
    const sk = document.getElementById('alpaca-secret'); if (sk) sk.value = cfg.alpaca.secret_key || '';
    const pp = document.getElementById('alpaca-paper'); if (pp) pp.checked = cfg.alpaca.paper !== false;
  }
  if (cfg.ibkr) {
    const h = document.getElementById('ibkr-host'); if (h) h.value = cfg.ibkr.host || '';
    const p = document.getElementById('ibkr-port'); if (p) p.value = cfg.ibkr.port || '';
    const c = document.getElementById('ibkr-client-id'); if (c) c.value = cfg.ibkr.client_id || '';
  }
...
function updateTickerTabs() {
  Array.from(document.getElementById('ticker-tabs').children).forEach(b => {
    b.classList.toggle('active', b.textContent === currentTicker);
  });
}
  ...
  try {
    ...
    if (sl) {
      ...
      data.signals.forEach(s => {
        ...
        div.className = 'signal-item ' + (s.signal==='BUY'?'buy':'sell');
    ...
    if (ol) {
      ...
      data.orders.forEach(o => {
        const div = document.createElement('div');
        div.className = 'signal-item ' + (o.action==='BUY'?'buy':'sell');
...
function updateCredFields() {
  ...
  const c = document.getElementById('cred-entries');
  c.innerHTML = '';
  if (broker === 'Alpaca') {
    ...
    c.innerHTML = '<p>No credentials needed (webhook)</p>';
  }
  if (config.alpaca) {
  ...
  if (config.ibkr) {
    ...
    const cid = document.getElementById('ibkr-client-id'); if (cid) cid.value = config.ibkr.client_id || '';
  }
}
updateCredFields();
...
function buildConfig() {
  ...
  return {
    ...
    ibkr: broker === 'Interactive Brokers' ? {
      ...
      port: document.getElementById('ibkr-port')?.value || '7497',

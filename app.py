# -*- coding: utf-8 -*-
"""
TraderMoney v2.0.25 – Triple-A Professional Trading Terminal
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    ...
    def get_performance_report(self) -> dict:
        ...
        with self._lock:
            for b, lats in self._latency_per_broker.items():
                if lats:
                    broker_latency_avg[b] = round(float(np.mean(lats)), 2)
                else:
                    broker_latency_avg[b] = 0.0
        symbol_slippage_avg = {}
        with self._lock:
            for s, slips in self._slippage_per_symbol.items():
                if slips:
                    symbol_slippage_avg[s] = round(float(np.mean(slips)), 3)
                else:
                    symbol_slippage_avg[s] = 0.0
        return {
...
class AlpacaBroker(BaseBroker):
    ...
    def connect(self) -> bool:
        ...
        if not key or not secret:
            self._emit_error("Alpaca API Key/Secret missing.")
            return False
        ...
        try:
            ...
            if acc.status != "ACTIVE":
                self._emit_error(f"Account status {acc.status}")
                return False
            self._emit_log(f"Connected. Paper={paper}. Equity=${acc.equity}")
            self.start_heartbeat()
            return True
        except Exception as e:
            self._emit_error(f"Alpaca connection error: {e}")
            return False
    def get_account(self):
        if not self.api:
            return None
        try:
            acc = self.api.get_account()
            pos = len(self.api.list_positions())
    ...
    def submit_order(self, symbol, qty, side, order_type="market", sl_pct=None, tp_pct=None, sl_price=None, tp_price=None) -> bool:
        ...
        try:
            request_price = 0.0
            try:
                request_price = float(self.api.get_latest_trade(symbol).price)
            except:
                pass
            ...
            self._emit_error(f"Order failed: {e}")
            return False
    def close_all_positions(self):
        if self.api:
            try:
                ...
                self._emit_error(f"Kill error: {e}")
    def get_positions(self):
        if not self.api:
            return {}
        try:
    ...
    def stream_prices(self, symbols, callback):
        ...
        def run():
            try:
                ...
                self._emit_log(f"Stream warning: {e}")
...
class IBKRBroker(BaseBroker):
    ...
    def _ensure_loop(self):
        if self._loop is None or not self._loop.is_running():
            ...
            while (self._loop is None or not self._loop.is_running()) and waited < 5:
                ...
                waited += 0.3
            if self._loop is None or not self._loop.is_running():
                raise RuntimeError("IBKR event loop failed")
    def _run_coro(self, coro, timeout=15):
        return asyncio.run_coroutine_threadsafe(coro, self._loop).result(timeout=timeout)
    def connect(self) -> bool:
        creds = self.config.get("ibkr", {})
        host = creds.get("host","127.0.0.1").strip()
    ...
    def submit_order(self, symbol, qty, side, order_type="market", sl_pct=None, tp_pct=None, sl_price=None, tp_price=None) -> bool:
        ...
        try:
            ...
            latency = (time.perf_counter()-t0)*1000
            analytics_db.record_execution(trace_id, symbol, self.name, side, qty, 0,0, latency, "rejected")
            self._emit_error(f"Order error: {e}")
            return False
    def close_all_positions(self):
        if not self.is_connected():
            return
...
class TradierBroker(BaseBroker):
    ...
    def connect(self) -> bool:
        ...
        if not token or not self.account_id:
            self._emit_error("Tradier token/account missing")
            return False
        ...
        try:
            ...
            if r.status_code != 200:
                self._emit_error(f"HTTP {r.status_code}")
                return False
...
class BinanceBroker(BaseBroker):
    ...
    def connect(self) -> bool:
        ...
        if not api_key or not api_secret:
            self._emit_error("Binance API key/secret missing")
            return False
        try:
            ...
            if not acct.get("canTrade"):
                self._emit_error("Account cannot trade")
                return False
            self._emit_log(f"Connected (testnet={testnet})")
            self.start_heartbeat()
            return True
        except Exception as e:
            self._emit_error(f"Binance connection error: {e}")
            return False
    def get_account(self):
        if not self.client:
            return None
    ...
    def stream_prices(self, symbols, callback):
        ...
        def run():
            try:
                ...
                self._ws_client = SpotWebsocketStreamClient(stream_url=stream_url, on_message=on_msg)
                for s in sym_map:
...
class BybitBroker(BaseBroker):
    ...
    def connect(self) -> bool:
        ...
        testnet = creds.get("testnet",True)
        if not api_key or not api_secret:
            self._emit_error("Bybit API key/secret missing")
            return False
        try:
            ...
            if resp.get("retCode",-1) != 0:
                self._emit_error(f"Auth failed: {resp.get('retMsg')}")
                return False
            self._emit_log(f"Connected (testnet={testnet})")
            self.start_heartbeat()
            return True
...
class OKXBroker(BaseBroker):
    ...
    def connect(self) -> bool:
        ...
        if not api_key or not api_secret or not passphrase:
            self._emit_error("OKX credentials missing")
            return False
        try:
            ...
            if str(resp.get("code","-1")) != "0":
                self._emit_error(f"Auth failed: {resp.get('msg')}")
                return False
    ...
    def stream_prices(self, symbols, callback):
        ...
        def run():
            try:
                ...
                demo = self.config.get("okx",{}).get("demo",True)
                url = "wss://wspap.okx.com:8443/ws/v5/public?brokerId=9999" if demo else "wss://ws.okx.com:8443/ws/v5/public"
...
class SignalAnalyzer:
    ...
    def generate_signal(df, prev_fast, prev_slow, config, smc=None):
        ...
        smc_str = ""
        if smc:
            smc_str = f" | MSS:{smc.get('mss','–')} CHoCH:{smc.get('choch','–')} OBs:{len(smc.get('order_blocks',[]))}"
            ...
            if sig == "BUY":
                if pos <= 0:
                    if pos < 0:
                        ...
                        if ok:
                            if self.risk_manager:
                                self.risk_manager.release_asset_exposure(sym, price * abs(pos))
                ...
                if pos >= 0:
                    if pos > 0:
                        ...
                        if ok:
                            if self.risk_manager:
                                self.risk_manager.release_asset_exposure(sym, price * pos)
                            self.positions[sym] = 0
                        else:
                            return
                    ok = False
    ...
    try:
        ...
        for sym in all_syms:
            try:
                ...
                continue
        if not data_dict:
            return jsonify({"html":"<p style='color:var(--muted)'>No data available.</p>"})
...
# ==============================================================================
# FRONTEND HTML – v2.0.11 with TradingView Widget
# ==============================================================================
FRONTEND_HTML = r"""<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<title>TraderMoney 2.0.24</title>
<style>
...
#chart-container{flex:1; min-height: 450px; margin:10px; background:var(--card); border-radius:var(--radius); position:relative;}
...
<div id="sb">
  <h2><svg class="icon"><use href="#i-lightning"/></svg> TraderMoney <span id="lbadge" class="lbadge li">FREE</span><small style="color:var(--muted);font-size:.56rem;margin-left:2px;">v2.0.24</small></h2>
...
function loadChart(symbol){ if(!symbol) return; const container = document.getElementById('chart-container'); if(!container) return; // Remove previous widget if any if(chartWidget) { try { chartWidget.remove(); } catch(e){} chartWidget = null; } // Clear container container.innerHTML = ''; // Wait for TradingView library if(typeof TradingView === 'undefined'){ console.warn('TradingView not loaded yet, retrying in 500ms'); setTimeout(()=>loadChart(symbol),500); return; } // Create new widget chartWidget = new TradingView.widget({ "container_id": "chart-container", "width": "100%", "height": "100%", "symbol": symbol, "interval": document.getElementById('timeframe').value, "timezone": document.getElementById('tz').value, "theme": "dark", "style": "1", "locale": "en", "toolbar_bg": "#f1f3f6", "enable_publishing": false, "allow_symbol_change": true, "studies": ["RSI@tv-basicstudies"], "hide_side_toolbar": false, "autosize": true }); }

...
class DatabaseManager:
    ...
    def _init_tables(self):
        ...
        CREATE TABLE IF NOT EXISTS leaderboard (
            ...
            win_rate REAL, total_signals INTEGER, last_backtest TEXT
        );
        """)
        self.conn.commit()
    ...
    def get_chat_sessions(self) -> List[dict]:
        cur = self.conn.execute(
            "SELECT id,title,created FROM chat_sessions ORDER BY id DESC")
    ...
    def update_leaderboard(self, user_id: str, win_rate: float, total_signals: int):
        self._exec("INSERT OR REPLACE INTO leaderboard VALUES(?,?,?,?)",
                   (user_id, win_rate, total_signals,
                    datetime.now().strftime("%Y-%m-%d %H:%M:%S")))

    def get_leaderboard(self) -> List[dict]:
        cur = self.conn.execute(
            "SELECT user_id,win_rate,total_signals,last_backtest "
            "FROM leaderboard ORDER BY win_rate DESC")
        return [{"user_id":r[0][:6],"win_rate":r[1],
                 "total_signals":r[2],"last_backtest":r[3]} for r in cur]
...
_DEFAULT_CONFIG: dict = {
    ...
    "timezone": "UTC", "watchlist": "", "offline_mode": False,
    "news_sentiment": False, "device_uuid": str(uuid.uuid4()),
...
class IBKRBroker(BaseBroker):
    ...
    def connect(self) -> bool:
        ...
        if not host:
            ...
            return False
        try:
            port = int(port_str); cid = int(cid_str)
        except ValueError:
    ...
    def get_positions(self):
        if not self.is_connected(): return {}
        try:
            return {p.contract.symbol: int(p.position)
                    for p in self.ib.positions() if p.position != 0}
        except Exception:
            return {}
...
class TradierBroker(BaseBroker):
    ...
    def get_account(self):
        ...
        try:
            ...
            return {"equity":       float(bal.get("total_equity",        0)),
                    "pl":           0.0,
...
class OKXBroker(BaseBroker):
    ...
    def connect(self) -> bool:
        ...
        if not passphrase: self._emit_error("OKX Passphrase is missing."); return False
        try:
...
class SignalAnalyzer:
    ...
    @staticmethod
    def generate_signal(df: pd.DataFrame, prev_fast, prev_slow,
                        config: dict) -> Tuple[Optional[str], str, float]:
    ...
    @staticmethod
    def _confirm(df: pd.DataFrame, config: dict,
            ...
            if sig == "BUY":
                ...
                if pos >= 0:
                    ...
                    if ok:
                        ...
                        self._telegram(f"SELL {qty} {sym} @ ${price:.2f} "
                                       f"(conf: {conf:.2f})")
                    else:
...
def _call_chatanywhere(messages: List[dict], retries: int = 3) -> str:
    ...
    for attempt in range(retries):
        try:
            ...
            last_exc = e
            db.insert_log(f"[AI Chat] Timeout on attempt {attempt+1}/{retries}")
        except http_requests.exceptions.HTTPError as e:
            last_exc = e
            db.insert_log(f"[AI Chat] HTTP error on attempt {attempt+1}/{retries}: {e}")
        except Exception as e:
            last_exc = e
...
def api_watchlist():
    ...
    if tickers and is_internet_available():
        try:
            import yfinance as yf
            for t in tickers:
                try:
            ...
            db.insert_log(f"Watchlist error: {e}")
    ...
    try:
            ...
            try:
                ...
                for s in sigs:
                    if s["signal"] == "BUY" and position <= 0:
                        ...
                        entry_time  = s["time"]
                        cash        = 0.0
                        trades.append({"entry_time": s["time"], "exit_time": "",
                ...
                if position != 0 and sigs:
                    last_sig = sigs[-1]; ep = last_sig["price"]
        ...
        if portfolio:
            resp["portfolio"] = {
                "initial_cash": initial_cash,
                "final_cash":   round(cash, 2),
                "total_pnl":    round(sum(t["pnl"] for t in all_trades
                                          if t["type"] == "exit"), 2),
...
def monte_carlo():
    ...
    try:
        ...
        pnl_results = []
        for _ in range(runs):
            cash = 10_000; position = 0.0; entry_price = 0.0
            for sym in symbols:
                try:
                    df = yf.download(sym, period=f"{days}d",
                                      interval=config.get("timeframe", "1m"),
...
def export_backtest_csv():
    ...
    si = io.StringIO()
    w  = csv.writer(si)
    w.writerow(["Entry Time", "Exit Time", "Side",
...
def export_backtest_pdf():
    ...
    pdf.cell(0, 10, "Backtest Report", ln=True, align="C")
    pdf.ln(5)
...
def correlation_matrix():
    ...
    all_syms  = list(dict.fromkeys(tickers + watchlist))
    if not all_syms:
    ...
    try:
        ...
        html  += "<tr><th></th>" + "".join(
            f"<th style='padding:3px 7px;color:#D4AF37'>{s}</th>"
            for s in corr.columns) + "</tr>"
        for row_sym in corr.index:
            html += f"<tr><td style='padding:3px 7px;color:#D4AF37;font-weight:bold'>{row_sym}</td>"
            for col_sym in corr.columns:
                v  = corr.loc[row_sym, col_sym]
                r_ = int(max(0, min(255, 178 + (1 - v) * 77)))
                g_ = int(max(0, min(255, 34  +      v  * 200)))
                html += (f"<td style='padding:3px 7px;background:rgb({r_},{g_},34);"
                         f"color:#fff;text-align:center'>{v:.2f}</td>")
            html += "</tr>"
        html += "</table>"
        return jsonify({"html": html})
    except Exception as e:
        return jsonify({"html": f"<p style='color:red'>Correlation error: {e}</p>"})
...
def api_chat():
    ...
    if not CHATANYWHERE_API_KEY or CHATANYWHERE_API_KEY.startswith("sk-YOUR"):
        return jsonify({"reply": "AI Chat not configured (set CHATANYWHERE_API_KEY in app.py)."})
    ...
    try:
        ...
        db.insert_log("[AI Chat] All retries timed out")
        return jsonify({
            "reply": ("The AI service timed out after 3 attempts. "
                      "Please try again in a moment."),
            "session_id": session_id,
        })
    except http_requests.exceptions.HTTPError as e:
        db.insert_log(f"[AI Chat] HTTP error after retries: {e}")
        return jsonify({
            "reply": f"AI service returned an HTTP error: {e}. Please try again.",
            "session_id": session_id,
        })
    except Exception as e:
        db.insert_log(f"[AI Chat] Unavailable after retries: {e}")
        return jsonify({
...
#watchlist-panel{background:#111;border-radius:7px;padding:6px 8px;margin-top:4px;max-height:100px;overflow-y:auto;font-size:.78rem;}
...
#chat-topbar .title{color:var(--accent);font-weight:600;font-size:.92rem;}
...
<div id="sb">
  ...
  <label>Watchlist (comma-sep, prices update every 30 s)</label>
  <input id="watchlist" placeholder="SPY, QQQ, TSLA">
  ...
  <label><span class="cb"><input type="checkbox" id="udefqty" checked onchange="toggleDefQty()">
    <span class="cm"></span></span> Use fallback qty</label>
  ...
  <div class="r2">
    ...
    <button onclick="loadPreset()" style="width:auto;margin-top:0;padding:7px 10px;">Load</button>
  </div>

  <button class="ghost" style="margin-top:14px;" onclick="checkUpdate()">🔄 Check Updates</button>
  <button class="ghost" style="margin-top:6px;"  onclick="runBT()">⚗ Backtest All</button>
...
<div id="main">
  ...
  <div id="tab-backtest" class="tab">
    <div class="btp">
      <div style="display:flex;gap:7px;padding:10px;flex-wrap:wrap;">
        <button class="ghost" style="width:auto;padding:9px 16px;" onclick="runBT()">⚗ Run Backtest</button>
  ...
  <div id="tab-analysis" class="tab">
    <div style="padding:10px;flex-shrink:0;">
      <button class="ghost" style="width:auto;padding:8px 16px;" onclick="loadCorr()">🔄 Refresh Correlation Matrix (30 days)</button>
    </div>
    <div style="overflow:auto;flex:1;padding:10px;" id="corr-content">
      <p class="ph">Click Refresh to load the correlation matrix for your tickers + watchlist.</p>
    </div>
  </div>
  ...
  <div id="tab-help" class="tab">
    <div class="hb">
      ...
      <table class="bttbl"><tr><th>Shortcut</th><th>Action</th></tr>
      ...
      <h4>🏦 Broker Notes</h4>
      <ul>
        <li><b>Alpaca:</b> Key + Secret from alpaca.markets. Paper checkbox = paper account.</li>
        <li><b>IBKR:</b> TWS/Gateway running. Ports: 7497=TWS paper | 7496=TWS live | 4002=GW paper | 4001=GW live.</li>
        <li><b>Tradier:</b> Access Token + Account ID from developer.tradier.com. Sandbox checkbox.</li>
        <li><b>Binance:</b> Key + Secret from binance.com. Testnet checkbox.</li>
        <li><b>Bybit:</b> Key + Secret from bybit.com. Testnet checkbox. Requires pybit v5.</li>
        <li><b>OKX:</b> Key + Secret + Passphrase from okx.com. Demo checkbox.</li>
      </ul>
      <h4>🔑 License</h4>
      <p>Purchase from <a href="https://shafayrich.gumroad.com/l/ykaoov" target="_blank">Gumroad ↗</a>. Validates per-session only.</p>
      <h4>🏆 Leaderboard</h4>
...
function buildCfg(){
  ...
  return{
    ...
    license_key:gv('lickey',''),watchlist:gv('watchlist',''),
...
async function checkUpdate(){
  try{let d=await(await fetch('/api/update')).json();if(d.update_available){$('upd').style.display='block';$('udl').href=d.download_url;toast('Update available!','success');}else toast('Up to date!','success');}catch(e){}
}
...
function renderWatchlist(prices){
  const syms=(gv('watchlist')||'').split(',').map(s=>s.trim().toUpperCase()).filter(s=>s);
  const box=$('watchlist-panel');
  if(!syms.length){box.innerHTML='';return;}
  box.innerHTML=syms.map(sym=>`<div class="wl-item"><span>${sym}</span><span class="wl-price">${prices[sym]?'$'+fmt(prices[sym],4):'—'}</span></div>`).join('');
}
...
document.addEventListener('keydown',e=>{
  ...
  if(ctrl&&e.key>='1'&&e.key<='7'){
    e.preventDefault();switchTab(TABS[parseInt(e.key)-1]);
  }
});

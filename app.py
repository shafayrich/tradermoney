# -*- coding: utf-8 -*-
"""
TraderMoney v2.0.22 – Triple-A Professional Trading Terminal
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
<head>
<meta charset="UTF-8">
<title>TraderMoney 2.0.23</title>
<style>
...
/* Chart container: larger, flexible */
...
<div id="sb">
  <h2><svg class="icon"><use href="#i-lightning"/></svg> TraderMoney <span id="lbadge" class="lbadge li">FREE</span><small style="color:var(--muted);font-size:.56rem;margin-left:2px;">v2.0.23</small></h2>
...
<div id="main">
  ...
  <div id="tkbar"></div>
  <!-- Dashboard tab with chart container and logbar -->
...
function switchTab(name){ document.querySelectorAll('.tbtn').forEach(btn=>btn.classList.remove('active')); document.querySelectorAll('.tab').forEach(tab=>tab.classList.remove('active')); document.querySelector(`.tbtn[onclick*="${name}"]`).classList.add('active'); $(`tab-content-${name}`).classList.add('active'); if(name==='analytics') loadAnalytics(); if(name==='aichat' && !chatInited) initAIChat(); if(name==='dashboard') setTimeout(()=>{ if(typeof TradingView !== 'undefined' && $('chart-container') && !chartWidget) loadChart($('.tkchip.active')?.textContent || 'AAPL'); }, 200); }
...
function loadChart(symbol){ if(!symbol) return; const container = $('#chart-container'); if(!container) return; if(chartWidget) { try { chartWidget.remove(); } catch(e){} chartWidget = null; } container.innerHTML = ''; if(typeof TradingView === 'undefined'){ console.warn('TradingView not loaded yet'); setTimeout(()=>loadChart(symbol),500); return; } chartWidget = new TradingView.widget({ "container_id": "chart-container", "width": "100%", "height": "100%", "symbol": symbol, "interval": $('timeframe').value, "timezone": $('tz').value, "theme": "dark", "style": "1", "locale": "en", "toolbar_bg": "#f1f3f6", "enable_publishing": false, "allow_symbol_change": true, "studies": ["RSI@tv-basicstudies"], "hide_side_toolbar": false }); }
